...
void ObjectWubbaDub::WeakCallback (v8::WeakCallbackInfo <ObjectWubbaDub> const & data)
#line 90 "./src/util/object_wubba_dub.lzz"
                                                        {
    ObjectWubbaDub* wrap = data.GetParameter();
    assert(wrap->refs_ == 0);
    wrap->handle_.Reset();
    delete wrap;
}
#line 100 "./src/util/object_wubba_dub.lzz"
uint16_t ObjectWubbaDub::kNodeEmbedderId = 0x90de;
...
#line 131 "./src/objects/database.lzz"
  : ObjectWubbaDub (), db_handle (db_handle), open (true), busy (false), safe_ints (false), unsafe_mode (false), was_js_error (false), has_logger (logger->IsFunction()), iterators (0), addon (addon), logger (isolate, logger), stmts (), backups ()
...
#line 217 "./src/objects/database.lzz"
                             {
                Database* db = ObjectWubbaDub :: Unwrap <Database>(info.This());
                if ( info . Length ( ) <= ( 0 ) || ! info [ 0 ] -> IsString ( ) ) return ThrowTypeError ( "Expected " "first" " argument to be " "a string" ) ; v8 :: Local < v8 :: String > source = ( info [ 0 ] . As < v8 :: String > ( ) ) ;
...
#line 415 "./src/objects/database.lzz"
                             {
                info.GetReturnValue().Set( ObjectWubbaDub :: Unwrap <Database>(info.This())->open);
}
...
#line 66 "./src/objects/statement.lzz"
  : ObjectWubbaDub (), db (db), handle (handle), extras (new Extras(id)), alive (true), locked (false), bound (false), has_bind_map (false), safe_ints (db->GetState()->safe_ints), mode (Data::FLAT), returns_data (returns_data)
...
#line 85 "./src/objects/statement.lzz"
                ...
                assert(info.IsConstructCall());
                Database* db = ObjectWubbaDub :: Unwrap <Database>(addon->privileged_info->This());
...
#line 172 "./src/objects/statement.lzz"
                            {
                Statement * stmt = ObjectWubbaDub :: Unwrap < Statement > ( info . This ( ) ) ; if ( ! stmt -> returns_data ) return ThrowTypeError ( "This statement does not return data. Use run() instead" ) ; sqlite3_stmt * handle = stmt -> handle ; Database * db = stmt -> db ; if ( ! db -> GetState ( ) -> open ) return ThrowTypeError ( "The database connection is not open" ) ; if ( db -> GetState ( ) -> busy ) return ThrowTypeError ( "This database connection is busy executing a query" ) ; if ( stmt -> locked ) return ThrowTypeError ( "This statement is busy executing a query" ) ; const bool bound = stmt -> bound ; if ( ! bound ) { Binder binder ( handle ) ; if ( ! binder . Bind ( info , info . Length ( ) , stmt ) ) { sqlite3_clear_bindings ( handle ) ; return ; } ( ( void ) 0 ) ; } else if ( info . Length ( ) > 0 ) { return ThrowTypeError ( "This statement already has bound parameters" ) ; } ( ( void ) 0 ) ; db -> GetState ( ) -> busy = true ; v8 :: Isolate * isolate = info . GetIsolate ( ) ; if ( db -> Log ( isolate , handle ) ) { db -> GetState ( ) -> busy = false ; db -> ThrowDatabaseError ( ) ; if ( ! bound ) { sqlite3_clear_bindings ( handle ) ; } return ; } ( ( void ) 0 ) ;
...
#line 218 "./src/objects/statement.lzz"
                             {
                Statement* stmt = ObjectWubbaDub :: Unwrap <Statement>(info.This());
...
#line 19 "./src/objects/statement-iterator.lzz"
  : ObjectWubbaDub (), stmt (stmt), handle (stmt->handle), db_state (stmt->db->GetState()), bound (bound), safe_ints (stmt->safe_ints), mode (stmt->mode), alive (true), logged (!db_state->has_logger)
...
#line 57 "./src/objects/statement-iterator.lzz"
                             {
                StatementIterator* iter = ObjectWubbaDub :: Unwrap <StatementIterator>(info.This());
...
#line 41 "./src/objects/backup.lzz"
  : ObjectWubbaDub (), db (db), dest_handle (dest_handle), backup_handle (backup_handle), id (id), alive (true), unlink (unlink)
...
#line 98 "./src/objects/backup.lzz"
                                 {
                Backup* backup = ObjectWubbaDub :: Unwrap <Backup>(info.This());
